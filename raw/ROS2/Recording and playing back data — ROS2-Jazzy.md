---
title: "Recording and playing back data — ROS 2 Documentation: Jazzy  documentation"
...
tags:
  - "clippings"
---
**You're reading the documentation for an older, but still supported, version of ROS 2. For information on the latest version, please have a look at [Lyrical](https://docs.ros.org/en/lyrical/Tutorials/Beginner-CLI-Tools/Recording-And-Playing-Back-Data/Recording-And-Playing-Back-Data.html).**
...
If you need to install ROS 2, see the [Installation instructions](https://docs.ros.org/en/jazzy/Installation.html).

This tutorial talks about concepts covered in previous tutorials, like [nodes](https://docs.ros.org/en/jazzy/Tutorials/Beginner-CLI-Tools/Understanding-ROS2-Nodes/Understanding-ROS2-Nodes.html), [topics](https://docs.ros.org/en/jazzy/Tutorials/Beginner-CLI-Tools/Understanding-ROS2-Topics/Understanding-ROS2-Topics.html) and [services](https://docs.ros.org/en/jazzy/Tutorials/Beginner-CLI-Tools/Understanding-ROS2-Services/Understanding-ROS2-Services.html). It also uses the [turtlesim package](https://docs.ros.org/en/jazzy/Tutorials/Beginner-CLI-Tools/Introducing-Turtlesim/Introducing-Turtlesim.html) and [Service Introspection Demo](https://docs.ros.org/en/jazzy/Tutorials/Demos/Service-Introspection.html).
...
`ros2 bag` can record data from messages published to topics. To see the list of your system’s topics, open a new terminal and run the command:
...
![../../../_images/record.png](https://docs.ros.org/en/jazzy/_images/record.png)
...
![../../../_images/playback.png](https://docs.ros.org/en/jazzy/_images/playback.png)
...
You’ll be recording service data between `introspection_client` and `introspection_service`, then display and replay that same data later on. To record service data between service client and server, `Service Introspection` must be enabled on the node.

Let’s start `introspection_client` and `introspection_service` nodes and enable `Service Introspection`. You can see more details for [Service Introspection Demo](https://docs.ros.org/en/jazzy/Tutorials/Demos/Service-Introspection.html).
...
`ros2 bag` can only record data from available services. To see the list of your system’s services, open a new terminal and run the command:
...
To record service data, the following options are supported. Service data can be recorded with topics at the same time.
...
Now `ros2 bag` is recording the service data published on the `/add_two_ints` service. To stop the recording, enter `Ctrl+C` in the terminal.
...
Before replaying the bag file, enter `Ctrl+C` in the terminal where `introspection_client` is running. When `introspection_client` stops running, `introspection_service` also stops printing the result because there are no incoming requests.
...
You can record data passed on topics and services in your ROS 2 system using the `ros2 bag` command. Whether you’re sharing your work with others or introspecting your own experiments, it’s a great tool to know about.