---
title: "Design-time considerations — Onshape to robot latest documentation"
...
- **Mate connectors** should have special names (see below for details):
	...
	> - `closing_name`: to close a kinematic loop (see [Handling kinematic loops](https://onshape-to-robot.readthedocs.io/en/latest/kinematic_loops.html#kinematic-loops))
...
![_images/design.png](https://onshape-to-robot.readthedocs.io/en/latest/_images/design.png)
...
If you create a mate connector and name it `link_something`, the link corresponding to the instance on which it is attached will be named `something` in the resulting export.
...
![_images/frames.png](https://onshape-to-robot.readthedocs.io/en/latest/_images/frames.png)

Here is a document that can be used (be sure to turn on “composite parts” when inserting it, use the `frame` composite part): [Onshape frame part](https://cad.onshape.com/documents/7adc786257f47ce24706bb32/w/774dd3de6bd5bfd65fb4462b/e/c60f72b9088ac4e5058b8904?renderMode=0&uiState=67b64076077d3a02bf5e1c0f)

> [!note] Note
> The instance used for frame representation is only here for visualization purpose and is excluded from the robot. You can however include it by setting [draw\_frames](https://onshape-to-robot.readthedocs.io/en/latest/config.html#draw-frames) to `true` in the [config](https://onshape-to-robot.readthedocs.io/en/latest/config.html) file, mostly for debugging purposes.

## Joint frames

Joint frames are the ones you see in Onshape when you click on the joint in the tree on the left. Thus, they are always revolving around the z axis, or translating along the *z axis*.

![_images/zaxis.png](https://onshape-to-robot.readthedocs.io/en/latest/_images/zaxis.png)

## Fixed robot

If you want to export a robot that is fixed to the ground, use the “Fixed” feture of Onshape:

![_images/fixed.png](https://onshape-to-robot.readthedocs.io/en/latest/_images/fixed.png)
...
> 
> In that case, you might consider using multiple URDF files, or [adding a dummy base link](https://onshape-to-robot.readthedocs.io/en/latest/processor_dummy_base_link.html#processor-dummy-base-link). However, this will fix all the base links to the base link without freedom.

## Gear relations

Gear relations are exported by onshape-to-robot. Be sure to click the **source joint** first, and then the **target joint**. They will be exported as `<mimic>` in [URDF](https://onshape-to-robot.readthedocs.io/en/latest/exporter_urdf.html) and [SDF](https://onshape-to-robot.readthedocs.io/en/latest/exporter_sdf.html) formats, and as equality constraints in [MuJoCo](https://onshape-to-robot.readthedocs.io/en/latest/exporter_mujoco.html).

![_images/gear.png](https://onshape-to-robot.readthedocs.io/en/latest/_images/gear.png)
